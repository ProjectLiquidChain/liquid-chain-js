  ...
  describe('#setSignature', function () {
    context('invalid signature size', function () {
      it('should throw error', function () {
        ...
        (function () {
          tx.setSignature(Buffer.from('abcd', 'hex'));
        }).should.throw(Error);
      });
    });
    ...
    context('invalid signature', function () {
      it('should throw error', function () {
        ...
        (function () {
          tx.setSignature(Buffer.from('ab2be566e387719c1640fcd2d0e6032eb16e5a80bbb9d409278f238d38ebf97cf8fbc73bfcaa270e2508cb46a22c9aca46293aec7ad7c996911cf16d61bd3c0b', 'hex'));
        }).should.throw(Error);
      });
    });
  });
  ...
  describe('#deserialize', function() {
    context('full', function () {
      it('should return transaction', function () {
        ...
        tx.getFrom().toString().should.equal('LDH4MEPOJX3EGN3BLBTLEYXVHYCN3AVA7IOE772F3XGI6VNZHAP6GX5R');
        tx.getNonce().cmpn(123).should.equal(0);
        const signature = tx.getSignature();
        if (!signature) {
        ...
        tx.getData().compare(Buffer.from('hello world')).should.equal(0);
        tx.getGasPrice().cmpn(456).should.equal(0);
        tx.getGasLimit().cmpn(100000).should.equal(0);
      });
    });
    ...
    context('without signature', function () {
      it('should return transaction', function () {
        ...
        tx.getFrom().toString().should.equal('LDH4MEPOJX3EGN3BLBTLEYXVHYCN3AVA7IOE772F3XGI6VNZHAP6GX5R');
        tx.getNonce().cmpn(123).should.equal(0);
        expect(tx.getSignature()).to.be.null;
        const to = tx.getTo();
        if (!to) {
          throw Error('Missing to');
        }
